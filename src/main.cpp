...
int main(int argc, char* argv[]) {
  auto* run_manager=G4RunManagerFactory::CreateRunManager(
    G4RunManagerType::Default); //Have pointer to run manager now
  //Physics
  auto physics_list = new QGSP_BERT_HP;
  physics_list -> SetVerboseLevel(0);
  run_manager -> SetUserInitialization(physics_list);
  //run_manager -> SetUserInitialization(new ne697-GeantExample::DetectorConstruction);
  //run_manager -> SetUserInitialization(new ne697-GeantExample::ActionInitialization);

  //Check number of command-line args, which controls whether we run in visual mode or batch mode
